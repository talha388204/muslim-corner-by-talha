...
// Generates simple SVG covers with Bengali titles and optional authors.
// Usage: node scripts/generate-svg-covers.js
...
// Saimum series: saimum-01 .. saimum-56
for (let i = 1; i <= 56; i++) {
...
function makeSVG(title, author, category) {
  ...
  const authorEsc = (author||"").replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');

  // simple line-wrapping for titles (aim for up to 4 lines)
  ...
  const lineTSpans = lines.map((ln, idx) => {
    return `<tspan x="300" dy="${idx===0?0:56}" font-size="${idx===0?40:34}" font-weight="700">${ln}</tspan>`;
  }).join('');
  // normalize category text for keyword checks
  ...
  // pick gradients and accents per category
  ...
  // accents differ: series gets teal + gold elements, islamic literature gets soft decorative band and subtle pattern
